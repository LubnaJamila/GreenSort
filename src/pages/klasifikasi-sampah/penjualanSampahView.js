...
export default class PenjualanSampahView {
      ...
      this.sidebarCollapsed ? "collapsed" : ""
                ...
                <div class="content-body">
                    ...
                    <div style="display: flex; justify-content: flex-end; margin-top: 30px; gap: 15px;">
                        <a href="#/klasifikasi-sampah" class="btn btn-outline">
                        Kembali
                        </a>
                        <button type="submit" class="btn btn-primary classify-btn" id="submit-btn">
                        Ajukan Sampah
                        </button>
                    </div>
                    </form>
  ...
  bindElements() {
    ...
    // Debugging
    console.log("classifiedImage:", this.classifiedImage);
    console.log("classificationResult:", this.classificationResult);
  }
  ...
  bindEvents() {
    ...
    if (this.beratSampahInput) {
      this.beratSampahInput.addEventListener("input", (e) =>
        this.validateBeratSampah(e)
      );
    }
  ...
  loadPenjualanData() {
    try {
      ...
      console.log("Data dari localStorage:", penjualanData);

      // Debugging: Cek elemen DOM
      console.log("Elemen gambar:", this.classifiedImage);
      console.log("Elemen result:", this.classificationResult);
      console.log("Elemen kategori:", this.kategoriSampahInput);
      console.log("Elemen berat:", this.beratSampahInput);

      // Set data jika elemen ada
      if (this.classifiedImage && penjualanData.image) {
        this.classifiedImage.src = penjualanData.image;
        this.classifiedImage.onload = () => {
          console.log("Gambar berhasil dimuat");
        };
        this.classifiedImage.onerror = () => {
  ...
  validateForm() {
    ...
    if (
      this.beratSampahInput?.value &&
      parseFloat(this.beratSampahInput.value) <= 0
    ) {
      isValid = false;
      this.showFieldError(
        this.beratSampahInput,
  ...
  submitPenjualanDirect(formData) {
    ...
    try {
      ...
      let pengajuanList = JSON.parse(
        localStorage.getItem("pengajuanSampah") || "[]"
      );
